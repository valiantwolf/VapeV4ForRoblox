			...
			if call then
				...
				antihit:Clean(runService.Heartbeat:Connect(function()
					if oldroot and oldroot.Parent and antihitcloned then
						...
						oldroot.CFrame = antihitting and CFrame.new(clone.CFrame.X, lastypos + -100, clone.CFrame.Z) or clone.CFrame
					end
				end))
				repeat
					if entitylib.isAlive then
...
run(function()
	...
	damageboost = vape.Categories.Blatant:CreateModule({
		Name = "Damage Boost",
		Tooltip = "Gives you more speed when you take velocity",
		Function = function(callback)
			if callback then
...
run(function()
	...
	local effecthudcolor: table = {
		Color3.fromRGB(255, 255, 255),
		Color3.fromRGB(255, 0, 0),
		Color3.fromRGB(0, 143, 57),
		Color3.fromRGB(0, 175, 67)
	};
	local effecthudtexts: securetable = Performance.new();
	local effectholder: Frame = Instance.new('Frame');
	effectholder.Size = UDim2.new(0, 100, 0, 31);
	effectholder.BackgroundTransparency = 1;

	local listlayout = Instance.new('UIListLayout', effectholder);
	listlayout.Padding = UDim.new(0, 5);
	local getimage = function(Parent: Frame?): ()
		local Image: ImageLabel = nil;
		for i: number, v: Frame? in Parent:GetDescendants() do
	...
	local removetab = function(tabname: string): ()
		for i: number, v: string in effecthudtexts do
			if v == tabname then
				table.remove(effecthudtexts, i);
				break;
			end;
		end;
	end;
	local neweffecthud = function(args: table): ()
		if table.find(effecthudtexts, args.name) then
			return;
		end;
		if not effecthud.Enabled then 
			return; 
		end;
		local actualduration = args.duration or 5
		actualduration = tostring(actualduration)
		actualduration = actualduration:split('</b>')[1]
		actualduration = tonumber(actualduration)

		local miniduration = actualduration / 2
		local lowestduration = actualduration / 3.5
		local hud: Frame = Instance.new('Frame', effectholder);
		hud.BackgroundColor3 = Color3.fromRGB(0, 1, 0);
		hud.BackgroundTransparency = 0.5;
		hud.Size = UDim2.new(0, 176, 0, 57);

		Instance.new('UICorner', hud).CornerRadius = UDim.new(0, 5);

		local effectimage: ImageLabel = Instance.new('ImageLabel', hud);
		effectimage.Image = args.image or 'rbxassetid://119193362854848';
		effectimage.Size = UDim2.new(0, 30, 0, 30);
		effectimage.BackgroundTransparency = 1
		effectimage.Position = UDim2.new(0.057, 0, 0.228, 0);

		local effectframe: Frame = Instance.new('Frame', hud);
		effectframe.BackgroundColor3 = Color3.fromRGB(172, 172, 172);
		effectframe.Size = UDim2.new(0, 2, 0, 21);
		effectframe.Transparency = 0.4;
		effectframe.Position = UDim2.new(0.267, 0, 0.316, 0);

		local effectname: TextLabel = Instance.new('TextLabel', hud);
		effectname.BackgroundTransparency = 1;
		effectname.Size = UDim2.new(0, 108, 0, 20);
		effectname.Position = UDim2.new(0.341, 0, 0.228, 0);
		effectname.TextSize = 12;
		effectname.Text = args.name
		effectname.TextColor3 = Color3.fromRGB(255, 255, 255)
		effectname.TextXAlignment = 'Left';
		effectname.TextYAlignment = 'Top';
		effectname.Font = Enum.Font.ArimoBold;

		local effectdur: TextLabel = Instance.new('TextLabel', hud);
		effectdur.BackgroundTransparency = 1;
		effectdur.Size = UDim2.new(0, 108, 0, 20);
		effectdur.Position = UDim2.new(0.348, 0, 0.579, 0);
		effectdur.Text = actualduration
		effectdur.TextSize = 12;
		effectdur.TextColor3 = Color3.fromRGB(0, 255, 0)
		effectdur.TextXAlignment = 'Left';
		effectdur.TextYAlignment = 'Top';
		effectdur.Font = Enum.Font.ArimoBold;

		if args.instance then
			local connection: RBXScriptConnection;
			local connection2: RBXScriptConnection;
			connection = args.instance:GetPropertyChangedSignal('Text'):Connect(function()
				effectdur.Text = `{args.instance.ContentText} seconds`
				if tonumber(args.instance.ContentText) <= miniduration then
					effectdur.TextColor3 = Color3.new(1, 1, 0);
				end;
				if tonumber(args.instance.ContentText) <= lowestduration then
					effectdur.TextColor3 = Color3.fromRGB(255, 0, 0);
				end;
			end)
			connection2 = lplr.PlayerGui.StatusEffectHudScreen.StatusEffectHud.DescendantRemoving:Connect(function(v)
			...
			task.spawn(function()
				repeat
					...
					effectdur.Text = `{math.floor(actualduration)} seconds`
					task.wait(0.08);
				until (not effecthud.Enabled)
			end)
		end;
		table.insert(effecthudtexts, args.name)
	end;
	effecthud = vape.Legit:CreateModule({
		Name = 'EffectHud',
		Function = function(call)
			effectholder.Visible = call;
			if call then
				if not lplr.PlayerGui:FindFirstChild('StatusEffectHudScreen') then 
					repeat task.wait() until lplr.PlayerGui:FindFirstChild('StatusEffectHudScreen')
				end
				for i: number, v: Frame? in lplr.PlayerGui.StatusEffectHudScreen.StatusEffectHud:GetDescendants() do
					if v.ClassName == 'TextLabel' then
						neweffecthud({
							name = v.Parent.Name, 
							duration = v.ContentText, 
							instance = v,
							image = getimage(v.Parent)
						});
					end;
				end;
				table.insert(effecthud.Connections, lplr.PlayerGui.StatusEffectHudScreen.StatusEffectHud.DescendantAdded:Connect(function(v: TextLabel)
					if v.ClassName == 'TextLabel' then
						neweffecthud({
							name = v.Parent.Name, 
							duration = v.ContentText, 
	...
	effectholder.Parent = effecthud.GetCustomChildren();
end);
